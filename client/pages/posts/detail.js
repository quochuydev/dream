  ...
  return (
    <>
      ...
      >
        ...
        {fileList.map(e => <div key={e.uid}>{e.response && <a onClick={()=>{
            const newBody =  form.getFieldValue('body') + `<figure class="image"><img src="${e.response.url}"/></figure>`
            setData({...data, body: newBody });
          }}>{e.response.url}</a>}</div>
          )}
      </Modal>
    </>
  );
